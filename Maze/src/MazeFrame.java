...
public class MazeFrame extends JFrame {
	 ...
	 * 
	 * @param mazeData
	 *            for details about what should be in this array see class
	 *            comments in Maze.java
	 * @param entryLoc
	 *            the entry location of the maze
	 * @param exitLoc
	 *            the "exit" location of the maze PRE: 0 <= entryLoc.getRow() <
	 *            mazeData.length and 0 <= entryLoc.getCol() <
	 *            mazeData[0].length and 0 <= exitLoc.getRow() < mazeData.length
	 *            and 0 <= exitLoc.getCol() < mazeData[0].length
	 */
	public MazeFrame(boolean[][] mazeData, MazeCoord entryLoc, MazeCoord exitLoc) {
		...
		add(searchStatusLabel, BorderLayout.NORTH); // put label at the top of
													// the
													// frame
		maze = new Maze(mazeData, entryLoc, exitLoc);

		mazeComponent = new MazeComponent(maze);
		add(mazeComponent, BorderLayout.CENTER); // put maze display in the
													// middle
													// of the frame
	...
	class MazeKeyListener extends KeyAdapter { // inner class -- has access to
												// outer
		 ...
		 * 
		 * @param event
		 *            What the user typed. Ignored here.
		 */
		public void keyPressed(KeyEvent event) {
			System.out.println("DEBUG: key pressed");

			System.out.println("DEBUG: doing maze search. . . ");
			boolean success = maze.search(); // maze defined in enclosing
												// MazeFrame
